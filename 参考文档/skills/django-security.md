...
```django
{# Django 默认自动转义变量 — 安全 #}
{{ user_input }}  {# HTML 会被转义 #}

{# 仅对可信内容显式标记为安全 #}
{{ trusted_html|safe }}  {# 不会转义 #}

{# 使用模板过滤器安全处理 HTML #}
{{ user_input|escape }}  {# 等同于默认行为 #}
...
def validate_file_size(value):
    ...
    if filesize > 5 * 1024 * 1024:
        raise ValidationError('文件太大。最大允许 5MB。')
...
def protected_view(request):
    return Response({'message': '你已通过身份认证'})
```
...
| 检查项 | 说明 |
|--------|------|
| `DEBUG = False` | 生产环境切勿启用 DEBUG |
| 仅使用 HTTPS | 强制 SSL、安全 Cookie |
| 强密钥 | 使用环境变量存储 SECRET_KEY |
| 密码验证 | 启用所有密码验证器 |
| CSRF 防护 | 默认启用，切勿禁用 |
| XSS 防护 | Django 自动转义，用户输入不要使用 `|safe` |
| SQL 注入防护 | 使用 ORM，切勿在查询中拼接字符串 |
| 文件上传 | 验证文件类型和大小 |
| 速率限制 | 对 API 端点进行节流 |
