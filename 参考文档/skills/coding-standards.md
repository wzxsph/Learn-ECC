...
description: 跨项目编码约定基准：命名、可读性、不可变性和代码质量审查。使用详细的前端或后端技能获取框架特定模式。
origin: ECC
---
...
此技能是共享的基础层，而非详细的框架手册。

- 使用 `frontend-patterns` 获取 React、状态、表单、渲染和 UI 架构。
- 使用 `backend-patterns` 或 `api-design` 获取仓储/服务层、端点设计、验证和服务器特定问题。
- 当需要最短可重用规则层而非完整技能演练时，使用 `rules/common/coding-style.md`。
...
**记住**：代码质量是不可妥协的。清晰、可维护的代码支持快速开发和自信的重构。