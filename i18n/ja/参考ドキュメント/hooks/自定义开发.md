...
本ガイド介绍如何为ECC Hooksシステム開発自定义钩子，パッケージ括基础構造、ベストプラクティス和exit 0要件。
...
process.stdin.on('end', () => {
  ...
  const toolInput = input.tool_input;      // ツール特定参数
...
|--------|------|----------|
| 0 | 成功 | 継続実行，或非重要警告 |
| 2 | 阻止 | 仅用于PreToolUse的重要阻塞 |
| その他非零 | エラー | 仅記録ログ，**不要使用** |

### 为什么非零終了码不好？

如果钩子以非零終了码終了（除2外），Claude Code会将整个ツール调用标记为失敗，这可能导致：
- ツール実行被意外中断
- 用户体验受损
- 难以デバッグ的問題
...
interface HookInput {
  ...
  tool_input: {               // ツール输入参数
    ...
    file_path?: string;        // Edit/Write/Read: ファイル路径
...
对于must阻止ツール実行的情况（PreToolUse only）：
...
// 不正确的做法 - 不要输出その他内容到stdout
console.log('Some message');  // 这会破坏データ流
```
...
| 中断 | 钩子ない終了 | 确保总は输出到stdout |
| データ损坏 | 输出到stdout非JSON | 仅输出原始data |
| パフォーマンス問題 | 钩子太慢 | 最適化或改为非同期 |
...
4. **总は输出原始data到stdout** - 不要改变データ流
5. **使用非同期处理长时间操作** - 設定async: true
6. **跨平台路径处理** - 使用path.join
7. **テスト输出フォーマット** - 确保stdoutは原始JSON
