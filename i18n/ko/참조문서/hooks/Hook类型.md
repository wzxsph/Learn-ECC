...
**控制台로그검사（응답后검사修改파일中的console.log）：**
```json
{
  "matcher": "*",
  "hooks": [{
  ...
  "description": "검사修改파일中是否있음console.log"
}
```
...
```json
{
  ...
  "description": "배치형식化모든편집的JS/TS파일并进行유형검사"
}
```
...
#### 유형설명
SessionStart钩子在세션라이프사이클시작时触发，用于加载先前上下文和检测프로젝트状态。
...
```json
{
  ...
  "purpose": "在세션시작时加载있음界限的先前上下文并检测프로젝트状态",
  "blocking": false
}
```
...
| SessionStart | 세션시작 | 否 | 否 | 加载上下文、检测프로젝트 |
