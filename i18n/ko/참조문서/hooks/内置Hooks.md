...
- Pre-commit품질검사 - 실행품질검사：lint暂存파일、검증커밋정보형식、감지console.log/debugger/密钥
...
**描述**: 阻止수정linter/formatter설정파일

**기능**:
- 阻止.eslintrc、.prettierrc等수정
- 引导수정코드而非削弱설정
...
**描述**: 事实强制GateGuard：阻止각파일的首次편집/쓰기，요구在허용前진행调查
...
**유형**: SessionStart
**matcher**: *
**描述**: 加载先前上下文并감지新세션的패키지 관리자

**기능**:
- 加载있음界限的先前上下文
- 프로젝트状态감지
- 패키지 관리자감지（npm/pnpm/yarn/bun）
...
| ID | Event | 기능 |
|----|-------|------|
| session:start | SessionStart | 加载上下文和감지패키지 관리자 |
