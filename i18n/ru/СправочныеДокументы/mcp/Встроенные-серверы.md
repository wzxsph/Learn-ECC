...
ECC 项目在 `mcp-configs/mcp-servers.json` 中预置了多个常用 MCP сервер配置。
...
| `omega-memory` | process | 增强版持久化代理内存，Поддерживать语义搜索、多代理协作和知识图谱 |
...
| `exa-web-search` | process | Через Exa API 进行网页搜索和研究 |
| `context7` | process | 实时文档查询 - 与 /docs 命令和 documentation-lookup skill 配合使用 |
| `magic` | process | Magic UI 组件 |
| `evalview` | process | AI agent 回归Тестирование - 快照行为，检测工具调用和输出质量的回归 |

### 浏览器自动化

| 服务器 | Тип | Описание |
|--------|------|------|
| `playwright` | process | Через Playwright 进行浏览器自动化和Тестирование |
| `browserbase` | process | Browserbase 云浏览器会话 |
| `browser-use` | http | AI 浏览器代理，执行网页Задача |
...
| `token-optimizer` | process | Token 优化 - Через内容去重和压缩Реализовать 95%+ 上下文缩减 |
| `devfleet` | http | 多Оркестрация агентов - 在隔离的 worktrees 中调度并行 Claude Code agents |
| `laraplugins` | http | Laravel 插件发现 - 按关键字搜索包、健康分数、Laravel/PHP 版本兼容性 |

## 快速启用

复制Требуется的服务器配置到 `~/.claude.json` 的 `mcpServers` 节，替换 `YOUR_*_HERE` 占位符为实际值。

**Пример - 启用 GitHub 服务器：**
