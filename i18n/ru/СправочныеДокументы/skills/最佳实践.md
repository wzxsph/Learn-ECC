...
本部分涵盖编码标准、Обработка ошибок和通用开发Лучшие практики。
...
### НазначениеОписание
跨项目适用的基线编码约定，Включать命名、可读性、不可变性和Качество кода审查。

### Когда использовать
- 启动新项目或模块
- 审查Качество кода和可维护性
- 重构现有代码遵循约定
- 强制命名、格式或结构一致性
- 设置linting、格式化或Тип检查规则
- 纳入新贡献者编码约定
...
### НазначениеОписание
跨TypeScript、Python和Go的健壮Обработка ошибок模式，ВключатьТип化错误、错误边界、重试、断路器和用户友好错误消息。

### Когда использовать
- 为新模块或服务Спроектировать错误Тип或异常层次结构
- 为不可靠的外部依赖添加重试逻辑或断路器
- 审查API端点是否缺少Обработка ошибок
- Реализовать用户友好错误消息和反馈
- 调试级联失败或静默错误吞并

### 核心原则

1. **快速失败并大声** - 在边界处暴露错误
2. **Тип化错误优于字符串消息** - 错误是一等公民，有结构
3. **用户消息 ≠ 开发者消息** - 向用户显示友好文本，服务器端记录完整上下文
4. **永远不静默吞并错误** - 每个`catch`块Обязательно处理、重新抛出或记录
...
| `golang-patterns` | Go惯用模式和Лучшие практики |
| `rust-patterns` | Rust惯用模式和Лучшие практики |
| `kotlin-patterns` | Kotlin惯用模式和Лучшие практики |
| `cpp-coding-standards` | C++编码标准和Лучшие практики |
| `tdd-workflow` | Разработка через тестирование工作流 |
| `security-review` | Безопасность审查检查清单 |

---

## Автономные агенты与循环系统

### autonomous-agent-harness

**Назначение**: 将 Claude Code 转变为完全自主的代理系统
...
**Когда использовать**:
- 用户想要连续自主操作或定时Задача
- Требуется构建跨会话记忆的个人 AI 助手
- Требуется复制 Hermes、AutoGPT 等Автономные агентыФреймворки功能
- Требуется计算机控制与定时执行结合
...
**Назначение**: 自主循环模式和架构 - 从简单顺序管道到 RFC 驱动的多代理 DAG 系统

**循环模式谱系**:
| 模式 | 复杂度 | Сценарий применения |
|------|--------|----------|
| Sequential Pipeline | 低 | 日常开发Шаги、脚本化工作流 |
| NanoClaw REPL | 低 | 交互式持久会话 |
| Infinite Agentic Loop | 中 | 并行内容生成、规范驱动工作 |
| Continuous Claude PR Loop | 中 | 多日迭代项目，CI 门禁 |
| De-Sloppify Pattern | 附加 | 任何 Implementer Шаги后的质量清理 |
...
5. **De-Sloppify Pattern** - 添加专门的清理/重构Шаги
