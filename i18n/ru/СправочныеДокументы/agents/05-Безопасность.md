...
Безопасность Agent 专门负责Уязвимости безопасности检测、合规性审查和敏感信息保护。

## Список Agent

| Agent Название | Назначение | Используемая модель | Основные инструменты |
|------------|------|----------|----------|
| security-reviewer | Уязвимости безопасности检测和修复专家 | sonnet | Read, Write, Edit, Bash, Grep, Glob |
| silent-failure-hunter | 静默失败检测专家 | sonnet | Read, Grep, Glob, Bash |
| healthcare-reviewer | 医疗应用Код-ревью（PHI/临床Безопасность） | opus | Read, Grep, Glob |
...
### Название和Назначение
Уязвимости безопасности检测和修复专家。在编写处理用户输入、认证、API 端点或敏感数据的代码后主动使用。标记 secrets、SSRF、注入、不Безопасность加密和 OWASP Top 10 漏洞。

### 能力Описание
- 漏洞检测 - 识别 OWASP Top 10 和常见Безопасность问题
- Secrets 检测 - 发现硬编码 API 密钥、密码、tokens
- 输入Проверка - 确保所有用户输入被正确清理
- 认证/授权 - Проверка适当的访问控制
- 依赖Безопасность - 检查有漏洞的 npm 包
- БезопасностьЛучшие практики - 强制执行Безопасность编码模式
...
- Read: 读取代码
- Write: 写入Безопасность修复
- Edit: 编辑Безопасность修复
...
- healthcare-reviewer 处理医疗特定Безопасность问题
...
1. **注入** - 查询是否参数化？用户输入是否清理？ORM 是否Безопасность使用？
2. **认证破坏** - 密码是否哈希 (bcrypt/argon2)？JWT 是否Проверка？会话是否Безопасность？
3. **敏感数据** - HTTPS 是否强制？Secrets 是否在 env vars？PII 是否加密？日志是否清理？
4. **XXE** - XML 解析器是否Безопасность配置？外部实体是否禁用？
5. **访问控制破坏** - 每个路由是否检查 auth？CORS 是否正确配置？
6. **Безопасность配置错误** - 默认凭据是否更改？生产环境 debug 模式是否关闭？Безопасность头是否设置？
7. **XSS** - 输出是否转义？CSP 是否设置？Фреймворки是否自动转义？
8. **不Безопасность反序列化** - 用户输入是否Безопасность反序列化？
9. **已知漏洞** - 依赖是否最新？npm audit 是否Через？
10. **日志和监控不足** - Безопасность事件是否记录？是否配置警报？
...
| 用户输入的 shell 命令 | CRITICAL | 使用Безопасность API 或 execFile |
...
1. **纵深防御** - 多层Безопасность
2. **最小权限** - 所需最小权限
3. **Безопасность失败** - 错误不应暴露数据
4. **不信任输入** - Проверка和清理一切
5. **定期更新** - 保持依赖最新

### 常见误报

- `.env.example` 中的环境变量（不是实际 secrets）
- Тестирование文件中的Тестирование凭据（如果明确标记）
- 实际公开的公钥 API 密钥
- Используется для校验和的 SHA256/MD5（不是密码）
...
1. 详细报告记录
2. 立即通知项目负责人
3. ПредоставлятьБезопасность代码Пример
4. Проверка修复有效
5. 如果凭据暴露则轮换 secrets
...
### Название和Назначение
Проверить код中的静默失败、被吞掉的错误、错误的回退和缺失的错误传播。
...
#### 5. 缺失Обработка ошибок
- 网络/文件/数据库路径无超时或Обработка ошибок
- 事务工作无回滚
...
### Название和Назначение
审查医疗应用代码的临床Безопасность、CDSS 准确性、PHI 合规性和医疗数据完整性。专为 EMR/EHR、临床决策Поддерживать和健康信息系统而Спроектировать。

### 能力Описание
- CDSS 准确性 - Проверка药物相互作用逻辑、剂量Проверка规则和临床评分Реализовать
- PHI/PII 保护 - 扫描患者数据在日志、错误、响应、URL 和客户端存储中的暴露
- 临床数据完整性 - 确保审计跟踪、锁定记录和级联保护
- 医疗数据正确性 - Проверка ICD-10/SNOMED 映射、实验室参考范围和药物数据库条目
- 集成合规性 - Проверка HL7/FHIR 消息处理和错误恢复
...
### Сотрудничество с другими Agent
- security-reviewer 处理通用Уязвимости безопасности
- code-reviewer 处理Качество кода问题
...
#### CDSS 引擎
- [ ] 所有药物相互作用对产生正确警报（双向）
- [ ] 剂量Проверка规则在超出范围值时触发
- [ ] 临床评分匹配发布规范
- [ ] 无假阴性（漏掉的相互作用 = 患者Безопасность事件）
- [ ] 格式错误的输入产生错误，而非静默Через
...
### Название和Назначение
在发布前Проверка开源 fork 已完全清理。扫描泄露的 secrets、PII、内部引用和危险文件。使用 20+ 正则表达式模式。生成 PASS/FAIL/PASS-WITH-WARNINGS 报告。
...
- 配置完整性Проверка - Проверка .env.example
- Git 历史审计 - 检查泄露的凭据

### Сценарий применения
- 开源 fork 发布前
- 第三方代码审计前
- 代码发布前的Безопасность检查
...
#### Шаги 3: 内部引用扫描 (CRITICAL)

```
# 绝对路径到特定用户主Содержание
...
生成项目Содержание中的 `SANITIZATION_REPORT.md`：
...
- **始终**检查 git 历史，即使是新仓库
- **保持偏执** - 误报可接受，漏报不可
- 任何Категория的单一 CRITICAL 发现 = Общий FAIL
- 单独Предупреждение = PASS WITH WARNINGS（用户决定）
