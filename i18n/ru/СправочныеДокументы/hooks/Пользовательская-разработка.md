...
本指南介绍如何为ECC Система Hooks开发自定义钩子，Включать基础结构、Лучшие практики和exit 0要求。
...
**重要**: 所有钩子Обязательно在非关键错误时exit 0，避免意外阻塞工具执行。

| 退出码 | 含义 | Сценарий использования |
|--------|------|----------|
| 0 | 成功 | 继续执行，或非关键Предупреждение |
| 2 | 阻止 | 仅Используется дляPreToolUse的关键阻塞 |
| 其他非零 | 错误 | 仅记录日志，**不要使用** |
...
对于Обязательно阻止工具执行的情况（PreToolUse only）：
...
- Безопасность检查失败（如检测到密钥）
- 违反硬性策略
- 可能导致数据丢失的操作

### 何时不要使用阻止

- Стиль кода问题（使用Предупреждение代替）
- 非关键检查
- 建议类检查
...
```javascript
// 使用console.error输出Предупреждение（显示给用户）
console.error('[Hook] Warning: some issue detected');

// 使用console.log输出原始数据（Обязательно）
console.log(data);
...
| 问题 | 原因 | 解决方案 |
|------|------|----------|
| 工具被意外阻止 | 钩子exit非零（除2外） | 改用exit 0和stderrПредупреждение |
    ...
    "PreToolUse": [
      {
        "matcher": "Write",
        "hooks": [],
        "description": "禁用文档文件Предупреждение"
      }
    ]
  }
}
