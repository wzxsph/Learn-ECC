...
本文档介绍 `scripts/lib/` Содержание下的核心Библиотека вспомогательных функций。
...
跨平台的Библиотека вспомогательных функций,适Используется для Windows、macOS 和 Linux。
...
```javascript
const {
  getHomeDir,      // 获取用户主Содержание
  getClaudeDir,    // 获取 ~/.claude Содержание
  getSessionsDir,  // 获取 ~/.claude/session-data Содержание
  getTempDir,      // 获取临时Содержание
  ensureDir,       // 确保Содержание存在,不存在则创建
  findFiles        // 查找匹配模式的文件
} = require('./utils');
...
```javascript
const {
  getDateString,      // Назад YYYY-MM-DD 格式
  getTimeString,      // Назад HH:MM 格式
  getDateTimeString   // Назад YYYY-MM-DD HH:MM:SS 格式
} = require('./utils');
```
...
const {
  ...
  getGitRepoName,       // 获取 git 仓库Название
  getProjectName        // 获取项目Название
} = require('./utils');
...
```javascript
const {
  readFile,        // Безопасность读取文件,失败Назад null
  writeFile,       // 写入文件
  appendFile,      // 追加到文件
...
// 从 stdin 读取 JSON(Используется для hooks)
const input = await readStdinJson({ timeoutMs: 5000 });

// 输出到 stderr(对用户可见)
log('Debug message');

// 输出到 stdout(Назад给 Claude)
output({ result: 'data' });
```
...
`getRunCommand` 和 `getExecCommand` 会Проверка输入:
...
Управление сессиями库,Предоставлять会话的 CRUD 操作。
...
const {
  ...
  getAllSessions,            // 获取所有会话(Поддерживать分页和筛选)
...
Tmux + Git Worktree 编排器,Управлять复杂的开发环境。
...
MCP (Model Context Protocol) 配置Управлять。
...
| `hook-flags.js` | Hook 标志Управлять |
| `inspection.js` | 检查工具 |
| `orchestration-session.js` | 编排Управление сессиями |
| `resolve-ecc-root.js` | ECC 根Содержание解析 |
| `resolve-formatter.js` | 格式化器解析 |
| `session-aliases.js` | 会话别名Управлять |
