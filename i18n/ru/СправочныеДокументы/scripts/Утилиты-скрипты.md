...
本文档介绍 ECC (Everything Claude Code) 项目中的主要Вспомогательные скрипты。
...
- [catalog.js](#catalogjs) - 组件Содержание查看器
...
| 命令 | 脚本 | Описание |
|------|------|------|
| `install` | install-apply.js | 安装 ECC 内容到Цель |
...
| `doctor` | doctor.js | 诊断缺失或漂移的 ECC Управлять文件 |
| `repair` | repair.js | 恢复漂移或缺失的 ECC Управлять文件 |
...
| `session-inspect` | session-inspect.js | 从 dmux 或 Claude 历史Цель发出规范会话快照 |
| `loop-status` | loop-status.js | 检查 Claude 脚本中的过时循环唤醒和待处理工具结果 |
| `uninstall` | uninstall.js | 删除记录在 install-state 中的 ECC Управлять文件 |
...
ECC 安装运行时,保持传统语言安装入口点完整,同时将Цель特定的变更逻辑移入可Тестирование的 Node 代码。
...
| 变量 | 默认值 | Описание |
|------|--------|------|
| `CLAW_SESSION` | `default` | 初始会话Название |
...
| Eval Coverage | 评估覆盖 |
| Security Guardrails | Безопасность护栏 |
...
- `framework` - Фреймворки组件 (nextjs, react, vue, etc.)
- `capability` - 能力组件
- `agent` - 代理组件
- `skill` - 技能组件
...
2. 使用 TypeScript 编译器编译 `.opencode` Содержание下的代码
3. 输出到 `.opencode/dist`
