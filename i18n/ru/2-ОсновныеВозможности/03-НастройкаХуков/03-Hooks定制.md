...
学习 ECC [Hook](../../СправочныеДокументы/hooks/HookТип.md) 系统，Реализовать工作流自动化。

## Обзор

ECC Hooks 系统是事件驱动的自动化机制，在 Claude Code 工具执行的前后触发，Используется для强制Качество кода、早期发现错误和自动化重复检查。
...
在工具执行**之前**运行，Можно阻止（exit code 2）或Предупреждение（stderr 输出但不阻止）。
...
在工具执行**之后**运行，Можно分析输出但无法阻止工具执行。
...
在每次 Claude 响应后运行，Используется для批量处理、最终检查和会话状态持久化。
...
生命周期钩子，Используется для会话Начать和结束时的上下文加载和状态持久化。
...
process.stdin.on('end', () => {
  ...
  // Предупреждение（非阻塞）：写入stderr
  console.error('[Hook] Предупреждение信息');
...
**重要**：所有钩子Обязательно在非关键错误时 exit 0，避免意外阻塞工具执行。

| 退出码 | 含义 | Сценарий использования |
|--------|------|----------|
| 0 | 成功 | 继续执行，或非关键Предупреждение |
| 2 | 阻止 | 仅Используется для PreToolUse 的关键阻塞 |
| 其他非零 | 错误 | 仅记录日志，**不要使用** |
...
### 4.1 Предупреждение TODO/FIXME 注释
...
```json
{
  ...
  "description": "Предупреждение添加TODO/FIXME注释"
}
```
...
```json
{
  ...
  "description": "添加新源文件时提醒创建Тестирование"
}
```
...
- 异步钩子运行在后台
- 无法阻塞工具执行
- Следует在 30 秒内Завершить
- 适Используется для：日志、分析、遥测
...
| 问题 | 原因 | 解决方案 |
|------|------|----------|
| 工具被意外阻止 | 钩子 exit 非零（除 2 外） | 改用 exit 0 和 stderr Предупреждение |
