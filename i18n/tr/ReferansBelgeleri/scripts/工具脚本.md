...
本文档介绍 ECC (Everything Claude Code) 项目中的主要Araç Betikleri。
...
- [catalog.js](#catalogjs) - 组件Dizin查看器
...
| 命令 | 脚本 | Açıklama |
|------|------|------|
| `install` | install-apply.js | 安装 ECC 内容到Hedef |
...
| `doctor` | doctor.js | 诊断缺失或漂移的 ECC Yönet文件 |
| `repair` | repair.js | 恢复漂移或缺失的 ECC Yönet文件 |
...
| `session-inspect` | session-inspect.js | 从 dmux 或 Claude 历史Hedef发出规范会话快照 |
| `loop-status` | loop-status.js | 检查 Claude 脚本中的过时循环唤醒和待处理工具结果 |
| `uninstall` | uninstall.js | 删除记录在 install-state 中的 ECC Yönet文件 |
...
ECC 安装运行时,保持传统语言安装入口点完整,同时将Hedef特定的变更逻辑移入可Test的 Node 代码。
...
| Eval Coverage | 评估覆盖 |
| Security Guardrails | Güvenlik护栏 |
...
- `framework` - Çerçeveler组件 (nextjs, react, vue, etc.)
- `capability` - 能力组件
- `agent` - 代理组件
- `skill` - 技能组件
...
1. 清理 `dist` Dizin
2. 使用 TypeScript 编译器编译 `.opencode` Dizin下的代码
3. 输出到 `.opencode/dist`

### Ön Koşullar

Gerekli已安装根Dizin的开发依赖,确保 TypeScript 编译器可用。
