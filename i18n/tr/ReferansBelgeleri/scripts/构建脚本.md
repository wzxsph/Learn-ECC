...
1. **版本Doğrulama**
   - 确认Sağla了 semver 格式版本号 (X.Y.Z 或 X.Y.Z-prerelease)
   - 确认当前分支是 main
   - 确认工作区干净
...
| `docs/SELECTIVE-INSTALL-ARCHITECTURE.md` | repoVersion Örnek |
...
3. 使用项目根Dizin的 tsconfig.json 编译 `.opencode` Dizin
...
GAN (General Ajan Network) Test工具相关。
