...
本文档介绍 `scripts/lib/` Dizin下的核心Araç Fonksiyon Kütüphanesi。
...
跨平台的Araç Fonksiyon Kütüphanesi,适Kullanılan Windows、macOS 和 Linux。
...
```javascript
const {
  getHomeDir,      // 获取用户主Dizin
  getClaudeDir,    // 获取 ~/.claude Dizin
  getSessionsDir,  // 获取 ~/.claude/session-data Dizin
  getTempDir,      // 获取临时Dizin
  ensureDir,       // 确保Dizin存在,不存在则创建
  findFiles        // 查找匹配模式的文件
} = require('./utils');
...
```javascript
const {
  getDateString,      // Geri Dön YYYY-MM-DD 格式
  getTimeString,      // Geri Dön HH:MM 格式
  getDateTimeString   // Geri Dön YYYY-MM-DD HH:MM:SS 格式
} = require('./utils');
```
...
const {
  ...
  getGitRepoName,       // 获取 git 仓库Ad
  getProjectName        // 获取项目Ad
} = require('./utils');
...
```javascript
const {
  readFile,        // Güvenlik读取文件,失败Geri Dön null
  writeFile,       // 写入文件
  appendFile,      // 追加到文件
...
// 从 stdin 读取 JSON(Kullanılan hooks)
const input = await readStdinJson({ timeoutMs: 5000 });

// 输出到 stderr(对用户可见)
log('Debug message');

// 输出到 stdout(Geri Dön给 Claude)
output({ result: 'data' });
```
...
`getRunCommand` 和 `getExecCommand` 会Doğrulama输入:
...
Oturum Yönetimi库,Sağla会话的 CRUD 操作。
...
const {
  ...
  getAllSessions,            // 获取所有会话(Destekle分页和筛选)
...
Tmux + Git Worktree 编排器,Yönet复杂的开发环境。
...
MCP (Model Context Protocol) 配置Yönet。
...
| 文件 | Açıklama |
|------|------|
| `agent-compress.js` | Ajan 上下文压缩 |
| `cost-estimate.js` | 成本估算 |
| `github-discussions.js` | GitHub 讨论集成 |
| `hook-flags.js` | Hook 标志Yönet |
| `inspection.js` | 检查工具 |
| `orchestration-session.js` | 编排Oturum Yönetimi |
| `resolve-ecc-root.js` | ECC 根Dizin解析 |
| `resolve-formatter.js` | 格式化器解析 |
| `session-aliases.js` | 会话别名Yönet |
