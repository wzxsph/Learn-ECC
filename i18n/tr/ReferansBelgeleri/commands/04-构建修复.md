...
本文档介绍 ECC 中Kullanılan修复各语言构建错误的专门命令。

---

## /go-build

**KullanımAçıklama**: 逐步修复 Go 构建错误、go vet Uyarı和 linter 问题。
...
3. **逐步修复** - 一次一个错误
4. **Doğrulama修复** - 每次更改后重新Yapılandırmayı Çalıştır
5. **报告总结** - 显示已修复和剩余问题
...
| 错误 | 典型修复 |
|------|----------|
| `tanımsız: X` | 添加导入或修复拼写 |
| `cannot use X as Y` | Tür转换或修复赋值 |
...
**KullanımAçıklama**: 逐步修复 Kotlin/Gradle 构建错误、编译器Uyarı和依赖问题。
...
**KullanımAçıklama**: 逐步修复 Rust 构建错误、借用检查器问题和依赖问题。
...
| `does not live long enough` | 使用拥有Tür或添加生命周期标注 |
| `cannot move out of` | 重构以获取所有权；仅作为最后手段克隆 |
| `mismatched types` | 添加 `.into()`、`as` 或显式转换 |
| `trait X not implemented` | 添加 `#[derive(Trait)]` 或手动Gerçekleştir |
...
**KullanımAçıklama**: 逐步修复 C++ 构建错误、CMake 问题和链接器问题。
...
| `no matching function` | 修复参数Tür或添加重载 |
| `tanımsız reference` | 链接库或添加Gerçekleştir |
| `multiple definition` | 使用 `inline` 或移到 .cpp |
| `incomplete type` | 用 `#include` 替换前向声明 |
| `no member named X` | 修复成员Ad或添加 include |
...
**KullanımAçıklama**: 修复 Android 和 Kotlin Multiplatform (KMP) 项目的 Gradle 构建错误。
...
| Expect 声明无 actual | 在每个平台源集中添加 `actual` Gerçekleştir |
...
| 配置缓存问题 | 检查非可序列化的Görev输入 |

---

## /flutter-build

**KullanımAçıklama**: 逐步修复 Dart 分析器错误和 Flutter 构建失败。
...
| `Missing concrete implementation of 'X'` | Gerçekleştir缺失的接口方法 |
...
| 命令 | 语言/平台 | 主要工具 | 常见问题 |
|------|----------|----------|----------|
| `/go-build` | Go | go build, go vet | Tür错误、导入循环 |
| `/kotlin-build` | Kotlin | Gradle, detekt | Tür不匹配、when 非穷举 |
| `/rust-build` | Rust | cargo check, clippy | 借用错误、生命周期 |
| `/cpp-build` | C++ | CMake, clang-tidy | 链接器错误、Şablon问题 |
| `/gradle-build` | Android/KMP | Gradle | 依赖冲突、配置错误 |
| `/flutter-build` | Flutter/Dart | Flutter analyze | 空Güvenlik、分析错误 |

---

## 通用Onarım Stratejisi

所有Yapı Onarımı命令遵循相同的基本策略：

1. **先Yapı Hatalarını Onarma** - 代码Zorunlu编译
2. **其次修复 lint Uyarı** - 修复可疑构造
3. **然后修复格式Uyarı** - 样式和En İyi Uygulamalar
4. **一次修复一个** - Doğrulama每次更改
5. **最小更改** - 不要重构，只是修复
