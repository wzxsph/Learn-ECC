...
Güvenlik Ajan 专门负责Güvenlik Açıkları检测、合规性审查和敏感信息保护。

## Ajan Listesi

| Ajan Ad | Kullanım | Kullanım Modeli | Temel Araçlar |
|------------|------|----------|----------|
| security-reviewer | Güvenlik Açıkları检测和修复专家 | sonnet | Read, Write, Edit, Bash, Grep, Glob |
| silent-failure-hunter | 静默失败检测专家 | sonnet | Read, Grep, Glob, Bash |
| healthcare-reviewer | 医疗应用Kod İnceleme（PHI/临床Güvenlik） | opus | Read, Grep, Glob |
...
### Ad和Kullanım
Güvenlik Açıkları检测和修复专家。在编写处理用户输入、认证、API 端点或敏感数据的代码后主动使用。标记 secrets、SSRF、注入、不Güvenlik加密和 OWASP Top 10 漏洞。

### 能力Açıklama
- 漏洞检测 - 识别 OWASP Top 10 和常见Güvenlik问题
- Secrets 检测 - 发现硬编码 API 密钥、密码、tokens
- 输入Doğrulama - 确保所有用户输入被正确清理
- 认证/授权 - Doğrulama适当的访问控制
- 依赖Güvenlik - 检查有漏洞的 npm 包
- GüvenlikEn İyi Uygulamalar - 强制执行Güvenlik编码模式
...
- healthcare-reviewer 处理医疗特定Güvenlik问题
...
1. **注入** - 查询是否参数化？用户输入是否清理？ORM 是否Güvenlik使用？
2. **认证破坏** - 密码是否哈希 (bcrypt/argon2)？JWT 是否Doğrulama？会话是否Güvenlik？
3. **敏感数据** - HTTPS 是否强制？Secrets 是否在 env vars？PII 是否加密？日志是否清理？
4. **XXE** - XML 解析器是否Güvenlik配置？外部实体是否禁用？
5. **访问控制破坏** - 每个路由是否检查 auth？CORS 是否正确配置？
6. **Güvenlik配置错误** - 默认凭据是否更改？生产环境 debug 模式是否关闭？Güvenlik头是否设置？
7. **XSS** - 输出是否转义？CSP 是否设置？Çerçeveler是否自动转义？
8. **不Güvenlik反序列化** - 用户输入是否Güvenlik反序列化？
9. **已知漏洞** - 依赖是否最新？npm audit 是否Vasıtasıyla？
10. **日志和监控不足** - Güvenlik事件是否记录？是否配置警报？
...
| 用户输入的 shell 命令 | CRITICAL | 使用Güvenlik API 或 execFile |
...
1. **纵深防御** - 多层Güvenlik
2. **最小权限** - 所需最小权限
3. **Güvenlik失败** - 错误不应暴露数据
4. **不信任输入** - Doğrulama和清理一切
5. **定期更新** - 保持依赖最新

### 常见误报

- `.env.example` 中的环境变量（不是实际 secrets）
- Test文件中的Test凭据（如果明确标记）
- 实际公开的公钥 API 密钥
- Kullanılan校验和的 SHA256/MD5（不是密码）
...
### Ad和Kullanım
Kodu İncele中的静默失败、被吞掉的错误、错误的回退和缺失的错误传播。
...
### Ad和Kullanım
审查医疗应用代码的临床Güvenlik、CDSS 准确性、PHI 合规性和医疗数据完整性。专为 EMR/EHR、临床决策Destekle和健康信息系统而Tasarım。

### 能力Açıklama
- CDSS 准确性 - Doğrulama药物相互作用逻辑、剂量Doğrulama规则和临床评分Gerçekleştir
- PHI/PII 保护 - 扫描患者数据在日志、错误、响应、URL 和客户端存储中的暴露
- 临床数据完整性 - 确保审计跟踪、锁定记录和级联保护
- 医疗数据正确性 - Doğrulama ICD-10/SNOMED 映射、实验室参考范围和药物数据库条目
- 集成合规性 - Doğrulama HL7/FHIR 消息处理和错误恢复
...
### 与其他 Ajan 的协作方式
- security-reviewer 处理通用Güvenlik Açıkları
- code-reviewer 处理Kod Kalitesi问题
...
#### CDSS 引擎
- [ ] 所有药物相互作用对产生正确警报（双向）
- [ ] 剂量Doğrulama规则在超出范围值时触发
- [ ] 临床评分匹配发布规范
- [ ] 无假阴性（漏掉的相互作用 = 患者Güvenlik事件）
- [ ] 格式错误的输入产生错误，而非静默Vasıtasıyla
...
### Clinical Accuracy
- CDSS: [检查Vasıtasıyla/失败]
- Drug DB: [Doğrulama/问题]
- Scoring: [匹配规范/偏离]
...
### Issues
1. [PATIENT SAFETY / CLINICAL / PHI / TECHNICAL] Tanım
   - Impact: [潜在危害或暴露]
   - Fix: [所需变更]
...
### Ad和Kullanım
在发布前Doğrulama开源 fork 已完全清理。扫描泄露的 secrets、PII、内部引用和危险文件。使用 20+ 正则表达式模式。生成 PASS/FAIL/PASS-WITH-WARNINGS 报告。
...
- 配置完整性Doğrulama - Doğrulama .env.example
- Git 历史审计 - 检查泄露的凭据
...
生成项目Dizin中的 `SANITIZATION_REPORT.md`：
...
- **始终**检查 git 历史，即使是新仓库
- **保持偏执** - 误报可接受，漏报不可
- 任何Kategori的单一 CRITICAL 发现 = Genel FAIL
- 单独Uyarı = PASS WITH WARNINGS（用户决定）
