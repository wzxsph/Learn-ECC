...
本部分涵盖编码标准、Hata Yönetimi和通用开发En İyi Uygulamalar。
...
### KullanımAçıklama
跨项目适用的基线编码约定，İçerir命名、可读性、不可变性和Kod Kalitesi审查。
...
### KullanımAçıklama
跨TypeScript、Python和Go的健壮Hata Yönetimi模式，İçerirTür化错误、错误边界、重试、断路器和用户友好错误消息。

### Kullanım Zamanı
- 为新模块或服务Tasarım错误Tür或异常层次结构
- 为不可靠的外部依赖添加重试逻辑或断路器
- 审查API端点是否缺少Hata Yönetimi
- Gerçekleştir用户友好错误消息和反馈
- 调试级联失败或静默错误吞并

### 核心原则

1. **快速失败并大声** - 在边界处暴露错误
2. **Tür化错误优于字符串消息** - 错误是一等公民，有结构
3. **用户消息 ≠ 开发者消息** - 向用户显示友好文本，服务器端记录完整上下文
4. **永远不静默吞并错误** - 每个`catch`块Zorunlu处理、重新抛出或记录
5. **错误是API合约的一部分** - 记录客户端可能收到的每个错误码

### TypeScriptHata Yönetimi
...
| `golang-patterns` | Go惯用模式和En İyi Uygulamalar |
| `rust-patterns` | Rust惯用模式和En İyi Uygulamalar |
| `kotlin-patterns` | Kotlin惯用模式和En İyi Uygulamalar |
| `cpp-coding-standards` | C++编码标准和En İyi Uygulamalar |
| `tdd-workflow` | Test Güdümlü Geliştirme工作流 |
...
**Kullanım**: 将 Claude Code 转变为完全自主的代理系统
...
**Kullanım Zamanı**:
- 用户想要连续自主操作或定时Görev
- Gerekli构建跨会话记忆的个人 AI 助手
- Gerekli复制 Hermes、AutoGPT 等Otonom AjanlarÇerçeveler功能
- Gerekli计算机控制与定时执行结合
...
**Kullanım**: 自主循环模式和架构 - 从简单顺序管道到 RFC 驱动的多代理 DAG 系统

**循环模式谱系**:
| 模式 | 复杂度 | Uygulama Senaryosu |
|------|--------|----------|
| Sequential Pipeline | 低 | 日常开发Adımlar、脚本化工作流 |
| NanoClaw REPL | 低 | 交互式持久会话 |
| Infinite Agentic Loop | 中 | 并行内容生成、规范驱动工作 |
| Continuous Claude PR Loop | 中 | 多日迭代项目，CI 门禁 |
| De-Sloppify Pattern | 附加 | 任何 Implementer Adımlar后的质量清理 |
...
5. **De-Sloppify Pattern** - 添加专门的清理/重构Adımlar
