...
## hooks.json 结构Genel Bakış

ECC Hooks Sistemi的配置文件采用JSON格式，主要结构如下：
...
| 字段 | Tür | Açıklama |
|------|------|------|
| `$schema` | string | JSON Schema URL，KullanılanDoğrulama配置 |
| `hooks` | object | 包含所有钩子Tür的对象 |

---

## 钩子数组结构

每个事件Tür（PreToolUse、PostToolUse等）包含一个钩子数组，每个钩子对象具有以下结构：
...
#### TürAçıklama
matcher字段Kullanılan指定触发钩子的工具Tür，Destekle单个工具或多个工具（用`|`分隔）。
...
```json
{
  ...
  "description": "Bash预检分发器，Kullanılan质量、tmux、推送和GateGuard检查",
...
```json
{
  ...
  "description": "加载先前上下文并检测新会话的包Yönet器",
  "id": "session:start"
}
```
...
| 字段 | Açıklama |
|------|------|
| `event` | 事件Tür（SessionStart、PreCompact、SessionEnd等） |
| `id` | 唯一标识符 |
| `script` | 脚本路径 |
| `purpose` | KullanımTanım |
...
ECC使用`run-with-flags.js`封装器来运行钩子，Destekle配置文件的运行时门控：
...
ECC的hook命令使用Node.js脚本Gerçekleştir跨平台行为：
