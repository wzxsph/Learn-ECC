...
ECC插件Sağla了丰富的内置HookGerçekleştir，位于`ECC/scripts/hooks/`Dizin中。
...
**Tür**: PreToolUse
**matcher**: Bash
**Tanım**: Bash预检分发器，整合了质量检查、tmux提醒、git推送提醒和GateGuard检查

**功能**:
- 开发服务器阻止器 - 阻止在tmux外运行`npm run dev`等命令
- Tmux提醒 - 建议对长时间运行的命令（npm test、cargo build、docker）使用tmux
- Git推送提醒 - 提醒在`git push`前审查更改
- Pre-commit质量检查 - 运行质量检查：lint暂存文件、Doğrulama提交信息格式、检测console.log/debugger/密钥
...
**Tür**: PreToolUse
**matcher**: Edit|Write
**Tanım**: 在逻辑间隔（约每50次工具调用）建议手动`/compact`
...
**Tanım**: 阻止修改linter/formatter配置文件
...
**Tanım**: 事实强制GateGuard：阻止每个文件的首次编辑/写入，要求在允许前进行调查
...
**Tür**: PostToolUse
**matcher**: Bash
**Tanım**: Bash后检分发器，Kullanılan日志记录、PR和构建通知
...
**Tür**: PostToolUse
**matcher**: Edit|Write|MultiEdit
**Tanım**: Uyarı前端编辑偏离为通用Şablon外观的UI
...
**Tür**: PostToolUse
**matcher**: Edit|Write|MultiEdit
**Tanım**: 记录编辑的JS/TS文件路径，以便在Stop时批量格式化和Tür检查
...
**Tür**: PostToolUse
**matcher**: *
**Tanım**: 在上下文耗尽、高成本、范围 creep 或工具循环时注入代理Uyarı
...
**matcher**: *
**Tanım**: 追踪失败的MCP工具调用，标记不健康的服务器并尝试重连
...
**Tür**: Stop
**matcher**: *
**Tanım**: 批量格式化（Biome/Prettier）和Tür检查（tsc）本次响应中编辑的所有JS/TS文件
...
**Tür**: Stop
**matcher**: *
**Tanım**: 每次响应后检查修改文件中的console.log
...
**Tür**: Stop
**matcher**: *
**Tanım**: 每次响应后持久化会话状态（Stop携带transcript_path）
...
**Tür**: Stop
**matcher**: *
**Tanım**: 在Claude响应时发送macOS/WSL桌面通知，包含Görev摘要
...
**Tür**: SessionStart
**matcher**: *
**Tanım**: 加载先前上下文并检测新会话的包Yönet器

**功能**:
- 加载有界限的先前上下文
- 项目状态检测
- 包Yönet器检测（npm/pnpm/yarn/bun）
...
| pre:write:doc-file-warning | Write | 文档文件Uyarı | 否 |
...
| post:edit:design-quality-check | Edit\|Write\|MultiEdit | Tasarım质量检查 | 否 |
| post:edit:accumulator | Edit\|Write\|MultiEdit | 编辑累积器 | 否 |
| post:edit:console-warn | Edit | console.logUyarı | 否 |
...
| ID | Matcher | 功能 | 超时 |
|----|---------|------|------|
| stop:format-typecheck | * | 批量格式化和Tür检查 | 300s |
...
| ID | Event | 功能 |
|----|-------|------|
| session:start | SessionStart | 加载上下文和检测包Yönet器 |
