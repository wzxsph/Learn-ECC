...
学习 ECC [Hook](../../ReferansBelgeleri/hooks/HookTür.md) 系统，Gerçekleştir工作流自动化。

## Genel Bakış

ECC Hooks 系统是事件驱动的自动化机制，在 Claude Code 工具执行的前后触发，Kullanılan强制Kod Kalitesi、早期发现错误和自动化重复检查。
...
在工具执行**之前**运行，Olabilir阻止（exit code 2）或Uyarı（stderr 输出但不阻止）。
...
在工具执行**之后**运行，Olabilir分析输出但无法阻止工具执行。
...
在每次 Claude 响应后运行，Kullanılan批量处理、最终检查和会话状态持久化。
...
生命周期钩子，Kullanılan会话Başla和结束时的上下文加载和状态持久化。
...
**重要**：所有钩子Zorunlu在非关键错误时 exit 0，避免意外阻塞工具执行。

| 退出码 | 含义 | Kullanım Senaryosu |
|--------|------|----------|
| 0 | 成功 | 继续执行，或非关键Uyarı |
| 2 | 阻止 | 仅Kullanılan PreToolUse 的关键阻塞 |
| 其他非零 | 错误 | 仅记录日志，**不要使用** |
...
| 问题 | 原因 | 解决方案 |
|------|------|----------|
| 工具被意外阻止 | 钩子 exit 非零（除 2 外） | 改用 exit 0 和 stderr Uyarı |
